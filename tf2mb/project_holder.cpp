...
sak::Project_Holder::Project_Holder(QString const& a_filepath):
  m_project{std::make_unique<Project>(a_filepath)},
  m_widget{}
  ...
  //m_unsaved_edits{false}
{
  ...
  m_widget = std::make_unique<Project_Main_Widget>(m_project.get());
}
...
void sak::Project_Holder::save() const
{
  ...
  QFile l_file{(this->project()->filepath())};
  ...
  if (l_file.open(QFile::WriteOnly | QFile::Truncate | QFile::Text))
  {
    ...
    // Read to it
    //Xml_Traits<Project>::to_stream(l_stream, *m_project);
...
void sak::Project_Holder::load()
{
  ...
  if (l_file.open(QFile::ReadOnly | QFile::Text))
  {
    ...
    //Xml_Traits<Project>::from_stream(l_stream, *m_project);
...
sak::Project* sak::Project_Holder::project() const
{
  return m_project.get();
}

sak::Project_Main_Widget* sak::Project_Holder::widget() const
{
  return m_widget.get();
}
