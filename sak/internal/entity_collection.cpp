...
// Special 6
//============================================================
sak::Entity_Collection::Entity_Collection() = default;
...
bool sak::Entity_Collection::is_empty() const
{
  return m_data.empty();
}

// How many active Entities are there?
std::size_t sak::Entity_Collection::count() const
{
  return m_data.size();
}
...
bool sak::Entity_Collection::has(Entity_ID a_id) const
{
  return do_has_id(m_data, a_id);
}

// Does this handle correspond to a currently active Entity?
bool sak::Entity_Collection::has_handle(Entity_Handle const& a_handle) const
{
...
std::size_t sak::Entity_Collection::index(Entity_ID a_id) const
{
...
// present the result is equal to count().
std::size_t sak::Entity_Collection::index_handle(Entity_Handle const& a_handle) const
{
  return static_cast<std::size_t>(std::distance(m_data.cbegin(), do_find_handle(m_data, a_handle)));
}

// Get all the ids for the data.
std::vector<sak::Entity_ID> sak::Entity_Collection::all_ids() const
{
...
std::vector<sak::Entity_Handle> sak::Entity_Collection::all_handles() const
{
  return m_data;
}
...
// is a null handle.
sak::Entity_Handle sak::Entity_Collection::get_handle(Entity_ID a_id) const
{
...
sak::Entity_ID sak::Entity_Collection::get_at(std::size_t a_index) const
{
...
sak::Entity_Handle sak::Entity_Collection::get_handle_at(std::size_t a_index) const
{
  if (a_index < m_data.size())
  {
...
std::vector<sak::Entity_ID> sak::Entity_Collection::get_all() const
{
  std::vector<Entity_ID> l_result{};
  l_result.reserve(m_data.size());
  for (auto const& l_handle : m_data)
...
std::vector<sak::Entity_Handle> sak::Entity_Collection::get_all_handles() const
{
...
bool sak::Entity_Collection::can_add(Entity_Handle const& a_handle) const
{
...
// at is returned.
std::size_t sak::Entity_Collection::add(Entity_Handle const& a_handle)
{
...
bool sak::Entity_Collection::can_remove(Entity_Handle const& a_handle) const
{
  return not_null(a_handle) && do_has_handle(m_data, a_handle);
}
...
std::size_t sak::Entity_Collection::remove(Entity_Handle const& a_handle)
{
