...
{
    qDebug() << "outliner::widget::impl::data_changed_at";
    // don't care about data
    if (a_section == 0)
    {
      auto l_files_item = m_root->file_header_item();
      //static_assert(std::is_same<decltype(m_root->file_header_item()), sak::file::outliner::header_item*>::value, "bad ptr");
      ...
      auto l_file_index = m_model.index(static_cast<int>(l_i),0,l_model_index);
...
{
    qDebug() << "outliner::widget::impl::added";
    auto l_files_item = m_root->file_header_item();
...
// When a File editor is to be opened, this is called.
void sak::project::outliner::widget::impl::requests_editor(file::extended_handle const&)
{
    qDebug() << "outliner::widget::impl::requests_editor";
    // Don't care.
}
