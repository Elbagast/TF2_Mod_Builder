...
// Get the objects at this index
typename sak::shared::project_access<sak::texture::object>::extended_handle_type sak::shared::project_access<sak::texture::object>::get_at(project::object& a_project, std::size_t a_index)
{
  return a_project.get_texture_at(a_index);
}

// Get all the objects
std::vector<typename sak::shared::project_access<sak::texture::object>::extended_handle_type> sak::shared::project_access<sak::texture::object>::get_all(project::object& a_project)
{
...
// Undoable add a new object using the supplied handle. If this handle is invalid or already in the data
// then nothing happens.
void sak::shared::project_access<sak::texture::object>::add(project::object& a_project, extended_handle_type const& a_ehandle)
{
  a_project.texture_add(a_ehandle);
}

// Undoable remove object. If this handle is invalid or not in the data nothing happens.
// Data is not deleted until the last reference is deleted.
void sak::shared::project_access<sak::texture::object>::remove(project::object* a_project, extended_handle_type const& a_ehandle)
{
  a_project->texture_remove(a_ehandle);
}

// Undoable change an object's maember value. If this handle is invalid or not in the data nothing happens.
// If the variant data is the wrong type for the member at the index nothing happens.
void sak::shared::project_access<sak::texture::object>::change_at(project::object* a_project, extended_handle_type const& a_ehandle, std::size_t a_index, typename object_type::member_value_variant const& a_variant)
{
  a_project->texture_change_at(a_ehandle, a_index, a_variant);
}

// Request that the editor for this texture be opened or switched to.
void sak::shared::project_access<sak::texture::object>::request_editor(project::object* a_project, extended_handle_type const& a_ehandle)
{
  a_project->texture_request_editor(a_ehandle);
}

// Request that the focus change to this object.
void sak::shared::project_access<sak::texture::object>::request_focus(project::object* a_project, extended_handle_type const& a_ehandle)
{
  a_project->texture_request_focus(a_ehandle);
}
